...
class Optimizer:
    ...
    def compute_sma(self, close_prices, window_sizes):
        """ Compute all SMA values at once using NumPy with uniform length output """

        # Find the minimum length for all SMAs (ensuring uniform shape)
        min_length = len(close_prices) - max(window_sizes) + 1
        if min_length <= 0:
            return np.array([])  # Return empty array if not enough data

        # Compute and trim each SMA to min_length
        sma_matrix = np.array([
            np.convolve(close_prices, np.ones(window) / window, mode='valid')[:min_length]
    ...
    @staticmethod
    @njit
    def compute_strategy(log_returns, sma_s, sma_l):
        """ Fast strategy backtesting using numba """
        position = np.where(sma_s > sma_l, 1, -1)
        strategy_returns = log_returns[1:] * position[:-1]  # Shifted position
        total_return = np.sum(strategy_returns)
        return total_return, strategy_returns

    def optimize_single_symbol(self, symbol):
        """ Optimize SMA parameters for a single symbol using vectorized operations """
        print(f"Processing {symbol}...")

        if symbol not in self.data or len(self.data[symbol]) < 200:  # Ensure enough data
        ...
        if len(close_prices) < 200:  # Ensure enough data points for SMA calculations
            return None

        log_returns = np.log(close_prices[1:] / close_prices[:-1])

        sma_s_range = np.arange(20, 51)
        sma_l_range = np.arange(100, 201, 10)

        # Precompute all SMAs
        sma_s_matrix = self.compute_sma(close_prices, sma_s_range)
        sma_l_matrix = self.compute_sma(close_prices, sma_l_range)

        # Skip processing if SMAs are empty (due to insufficient data)
        if sma_s_matrix.size == 0 or sma_l_matrix.size == 0:
            print(f"Skipping {symbol}: Not enough data for SMA calculations.")
            return None

            # Ensure log_returns is the same length as SMAs
        min_length = min(sma_s_matrix.shape[1], sma_l_matrix.shape[1], len(log_returns))
        log_returns = log_returns[-min_length:]  # Trim to match SMAs

        best_config = {"SMA_S": None, "SMA_L": None, "Performance": -np.inf, "Last_Signal": None}

        for i, SMA_S in enumerate(sma_s_range):
            for j, SMA_L in enumerate(sma_l_range):
                if SMA_S >= SMA_L:
                    continue

                sma_s = sma_s_matrix[i][-min_length:]
                sma_l = sma_l_matrix[j][-min_length:]

                total_return, strategy_returns = self.compute_strategy(log_returns, sma_s, sma_l)

                if total_return > best_config["Performance"]:
                    last_signal = "Buy" if sma_s[-1] > sma_l[-1] else "Sell"
                    best_config.update(
                        {"SMA_S": SMA_S, "SMA_L": SMA_L, "Performance": total_return, "Last_Signal": last_signal})

        if best_config["Performance"] > 0.20:
            return {
                "Symbol": symbol,
    ...
    def find_best_sma(self):
        """ Find the best SMA parameters for all symbols using parallel processing """
        results = Parallel(n_jobs=-1)(delayed(self.optimize_single_symbol)(symbol) for symbol in self.symbols)
